...
const BookingDetailModal = ({ booking, roomId, onClose, onCheckIn, onCheckOut, onDelete, onUpdate }) => {
  ...
  const effectiveBooking = useMemo(() => {
    ...
    const checkIn = booking.checkIn || historyMatch?.checkIn || historyMatch?.checkInDate || '2026-09-20';
    const checkOut = booking.checkOut || historyMatch?.checkOut || historyMatch?.checkOutDate || '2026-09-24';
    ...
    // 8. Trạng thái
    const status = booking.status || historyMatch?.status || 'đang ở';
