...
const playSpinSound = () => {
  ...
  notes.forEach((note, index) => {
    ...
    gain.gain.exponentialRampToValueAtTime(0.05, audioCtx.currentTime + index * 0.08 + 0.03);
    gain.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + index * 0.08 + 0.16);
    osc.connect(gain);
    gain.connect(master);
    osc.start(audioCtx.currentTime + index * 0.08);
    osc.stop(audioCtx.currentTime + index * 0.08 + 0.17);
  });

  setTimeout(() => audioCtx.close(), 700);
};

const playPrizeSound = (isLuckPrize) => {
  if (typeof window === 'undefined') return;
  ...
  const audioCtx = new AudioContextClass();
  const master = audioCtx.createGain();
  master.gain.value = isLuckPrize ? 0.055 : 0.07;
  master.connect(audioCtx.destination);

  const notes = isLuckPrize
    ? [220, 196, 174, 146.83]
    : [523.25, 659.25, 783.99, 1046.5, 1174.66];

  notes.forEach((note, index) => {
    const osc = audioCtx.createOscillator();
    const gain = audioCtx.createGain();
    osc.type = isLuckPrize ? 'triangle' : 'sine';
    osc.frequency.value = note;
    gain.gain.setValueAtTime(0.001, audioCtx.currentTime + index * 0.12);
    gain.gain.exponentialRampToValueAtTime(isLuckPrize ? 0.06 : 0.08, audioCtx.currentTime + index * 0.12 + 0.04);
    gain.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + index * 0.12 + (isLuckPrize ? 0.42 : 0.27));
    osc.connect(gain);
    gain.connect(master);
    osc.start(audioCtx.currentTime + index * 0.12);
    osc.stop(audioCtx.currentTime + index * 0.12 + (isLuckPrize ? 0.48 : 0.32));
  });
...
const LuckyWheel = ({ onClaimPrize }) => {
  ...
  useEffect(() => {
    if (!showPrizeModal || !wonPrize) return;
    playPrizeSound(wonPrize.type === 'luck');
              ...
              >
                <svg viewBox="0 0 100 100" className="w-full h-full">
                  {luckyWheelPrizes.map((prize, idx) => {
                    ...
                    const shortNameLines = getShortNameLines(prize.shortName);
                    ...
                    return (
                      <g key={prize.id}>
                        ...
                        <text
                          x={textX}
                          y={textY - (shortNameLines.length - 1) * 2.5}
                          fill={prize.textColor}
                          fontSize={shortNameLines.length > 2 ? 2.7 : 3.4}
                          fontWeight="bold"
                          textAnchor="middle"
                        ...
                        >
                          {shortNameLines.map((line, lineIndex) => (
                            <tspan
                              key={`${prize.id}-line-${lineIndex}`}
                              x={textX}
                              dy={lineIndex === 0 ? 0 : 4.2}
                            >
                              {line}
                            </tspan>
                          ))}
